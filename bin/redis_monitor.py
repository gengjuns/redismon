...
class RedisFalconMonitor(object):
    ...
    def ping_redis(self):
        ...
        if redis_is_alive == 0:  # If redis is dead, update the alive metrice here.
            redis_alive_data = [
                {"endpoint": "redis-" + self.addr, "metric": "redis_alive", "tags": self.tags, "timestamp": upload_ts,
    ...
    def send_data(self):
        ...
        for info_key in redis_info_dict.keys():
            ...
            if (info_key in calculate_keys or re.match("^cmdstat", info_key)):
                key_item_dict = {"endpoint": "redis-" + self.addr, "metric": info_key, "tags": self.tags,
                                 "timestamp": upload_ts, "value": redis_info_dict[info_key], "step": 60,
                                 "counterType": "COUNTER"}
            else:
                key_item_dict = {"endpoint": "redis-" + self.addr, "metric": info_key, "tags": self.tags,
                                 "timestamp": upload_ts, "value": redis_info_dict[info_key], "step": 60,
